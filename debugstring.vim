...
function! g:FilelocationPrefixStr()
  let l:debug_str = '[' . fnamemodify(bufname('%'), ':t') . ':'
  if getline('.') =~# '^$' " Empty line
      let l:debug_str .= line('.')
  else
...
if !hasmapto('<Plug>DumpDebugStringCur')
    ""@setting default_dump_debug_map
    "
    nmap <unique> <Leader>dc  <Plug>DumpDebugStringCur
endif

if !hasmapto('<Plug>DumpDebugStringVis')
    ""@setting default_dump_debug_map
    "
    nmap <unique> <Leader>dv  <Plug>DumpDebugStringVis
endif
...
function! s:debugFunctionWrapper(mode, ...)
    ...
    elseif a:mode ==# s:modes['cur_debug']
        if !exists(':AddDebugStringExpr')
            echoerr "Command AddDebugStringExpr isn't implemented for filetype \"" . &filetype . "\""
            return 0
        endif
    ...
    elseif a:mode ==# s:modes['vis_debug']
        if !exists(':AddDebugStringExpr')
            echoerr "Command AddDebugStringExpr isn't implemented for filetype \"" . &filetype . "\""
            return 0
        endif
...
nnoremap <silent> <Plug>DumpDebugStringCur :<C-U>call <SID>debugFunctionWrapper(2)<CR>
vnoremap <leader>dv :call <SID>debugFunctionWrapper(3)<cr>
