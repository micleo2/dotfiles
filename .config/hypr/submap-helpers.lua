...
local function submap_ipc_payload(submap)
	local parts = {}
...
local function dispatch_qs_submap(submap)
	hl.dispatch(hl.dsp.exec_cmd("qs -c retro ipc call submap display '" .. submap_ipc_payload(submap) .. "'"))
end

function M.define_submap(submap_name, submap_activation_key, submap_options_per_key)
	hl.bind(submap_activation_key, function()
		dispatch_qs_submap(submap_options_per_key)
